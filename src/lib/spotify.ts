...
export async function getPlaylistTracks(playlistId: string): Promise<EnrichedTrack[]> {
  ...
  }>(`/playlists/${playlistId}/tracks?limit=100&fields=items(track(id,name,artists,album,duration_ms,preview_url))`);
  ...
  return tracks.map((track) => {
    ...
    return {
      ...
      features: f,
      keyName: f && f.key >= 0
        ? `${KEY_NAMES[f.key]} ${f.mode === 1 ? "maj" : "min"}`
        : undefined,
